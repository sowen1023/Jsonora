...
Jsonora 会在 JSON 响应页自动格式化左侧 JSON，同时保留原始内容供复制和下载；图形/树形面板默认收起，需要时可从「视图」菜单展开。普通网页中的 JSON 代码块也能按需就地展开；界面提供中英文和多套明暗配色。

## 从这些场景开始

| 你正在做什么 | Jsonora 的入口 |
| --- | --- |
| 阅读 API 返回的 JSON | 直接打开接口地址，自动显示格式化后的 JSON；按需展开图形或树形 |
